...
//ALL TAKEN FROM GOBILDA VANILLA
    ...
    public void controlArmPreset(Gamepad gamepad) {
        ...
        if (gamepad.square) {
            setArmPosition(ARM_COLLAPSED); // Arm fully collapsed
        } else if (gamepad.circle) {
            setArmPosition(ARM_COLLECT); // Arm for collecting
        } else if (gamepad.triangle) {
            setArmPosition(ARM_CLEAR_BARRIER); // Arm raised to clear a barrier
        } else if (gamepad.cross) {
            setArmPosition(ARM_SCORE_SPECIMEN); // Arm for scoring a specimen
        }
